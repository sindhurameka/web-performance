...
var resizePizzas = function(size) {
  ...
  //Used document.getElementById insted of querySelector to improve performance
  ...
  function changePizzaSizes(size) {
    // document.getElementsByClassName("randomPizzaContainer") is assigned to selector variable and
    //the variable is used for performance improvement
    var selector = document.getElementsByClassName("randomPizzaContainer");
    var len = selector.length;
    //determineDx function is oustide the for-loop as it just needs to be calculated once
    var dx = determineDx(selector[0], size);
    //newWidth od the pizzas is outside the for-loop as it just needs to be calculated once
    var newwidth = (selector[0].offsetWidth + dx) + 'px';
    //new width is assigned to each of the pizzas using for-loop
    for (var i = 0; i < len; i++) {
...
function updatePositions() {
  ...
  //Using getElementsByClassName instead of querySelector for performance improvement
  //Assigned document.getElementsByClassName("mover"); to items variable and used
  var items = document.getElementsByClassName("mover");
  var itemLength = items.length;
  //The operation (document.body.scrollTop / 1250) is calculated only once out of the for-loop
  var phaseCount = document.body.scrollTop / 1250 ;
  /* The operation (i % 5) just has 5 values repeting (0,1,2,3,4) so for-loop is used with var j
     that loops and assigns the value */
  for(var j = 0 ; j < 5; j++)
  {
    var phase = Math.sin((phaseCount) + j);
    //for-loop for assigning phase value to each of the background pizzas for animation
...
//Based on the size of viewport the number of background pizzas are determined
if (window.matchMedia("(max-width: 480px)").matches){
  /* For small screen devices with the viewport width less than 480 px*/
  pizzaNumber = 10;

} else if(window.matchMedia("(max-width: 760px)").matches){
  /* For medium screen devices with the viewport width less than 760 px and greater than 480 */
  pizzaNumber = 20;
}else if(window.matchMedia("(min-width: 761px)").matches){
  /* For large screen devices with the viewport width greater than 760 px */
  pizzaNumber = 30;
}
